...
class ClientOptions:
    ...
    __slots__ = ('_camMap', '_rotation', '_zoomLocation', '_fps', '_iso',
                 ...
                 '_camSelects', '_camSelectsIsActive', '_fpsCamMap'
                 )
...
class CameraServer(socketserver.ThreadingMixIn, socketserver.TCPServer):
    ...
    def sendStartRecordingMessages(self) -> None:
        ...
        #    self.cageSettingsSet = False
        #elif not self.cageSettingsSet and self.cageSettingsOn:
        #    other = 0
        #    for other in self.settingsCagesStore:
        #        self.sendSetCagesMessages(other, self.cageMapper, self.settingsCagesStore)
        #    self.cageSettingsSet = True
        #    self.cameraSettingsSet = False
        #    self.cageSettingsOn = True
...
#****************************************************************Asynchronous Recording Methods******************************************************
    ...
    def sendStartRecordingMessagesCage(self) -> None:
        ...
        #self.cageMapper = copy.deepcopy(cageMap)
        if not cage:
...
#################################################################################################            
    def sendSplitRecordingMessagesCage(self, cage: str) -> None:
        ...
        #print("G                  List of Cams: {} ".format(listCamstoSplit))
    ...
    def sendStopRecordingMessagesCage(self) -> None:
        ...
        cage = self.cageSelect
        #self.cageMapper = copy.deepcopy(cageMap)
        #if self.cageMapper != cageMap:
        #    print("cageMap *** {}".format(cageMap))
        #    print(" ")
        #    print("self.cageMapper *** {}".format(self.cageMapper))
    ...
    def sendSetCagesMessages(self, cage: str, cageMap: Dict[str, Union[str, Dict, bool]], settingsCages: Dict[str, Union[str, int, float, Tuple, Dict, List, bool]]) -> None:
        ...
        print(self.clientOptions.zoomDimension)
        ...
        if bool(listCamsSet):
            for cam in listCamsSet:
                ...
                #self.sendToSelect('Camera', 'set FPS', cam)
    ...
    #def handlePreviewingError(self, client: 'ClientController', message: str) -> None:
...
class CameraServerController(threading.Thread):
    ...
    def toggleRecording(self, cageMap: Dict[str, Union[str, Dict, bool]]) -> None:
...
#****************************************************************Asynchronous Recording Methods******************************************************
    def startRecordingCage(self, cage: str, cageMap: Dict[str, Union[str, Dict, bool]]) -> None:
        try:
            self.recordStartTimeCage[cage] = time.time() + 1  # Make sure timers are individualized for the case that this isn't synchronized
            #self.recordStartTime = time.time() + 1
            #self.asyncQueue.call(self.server.sendStartRecordingMessagesCage)
            self.asyncQueue.call(self.server.sendStartRecordingMessagesCage)
            holder = threading.Timer(
                self.server.options.segmentSize*60, self.splitRecordingCage, args=(cage, cageMap,))
            self.splitSchedulerCage.update([(cage, holder)])
    ...
    def splitRecordingCage(self, cage: str, cageMap: Dict[str, Union[str, Dict, bool]]) -> None:
        try:
            ...
            holder = threading.Timer(self.server.options.segmentSize*60, self.splitRecordingCage, args=(cage, cageMap,))
            self.splitSchedulerCage.update([(cage, holder)])
            self.splitSchedulerCage[cage].start()
        except RecordingOrStreamException as err:
            logger.error(err)
        ...
        if self.synch:
            ...
            if cage and bool(cageMap):
                if self.server.cageMapper[cage]['isRecording']:
                    ...
                    #self.startRecordingCage(cage, cageMap)
        ...
        if cageMap:
            for cage in cageMap:
                if self.splitSchedulerCage:
                    if self.splitSchedulerCage is not None and self.splitSchedulerCage[cage].isAlive():
                        self.splitSchedulerCage[cage].cancel()
                if cageMap[cage]['isRecording']:
    ...
    def streamVideoToFile(self, filename: str, width: int, height: int) -> int:
        ...
        class StreamThread(threading.Thread):
            ...
            def downloadVideo(self) -> bool:
                ...
                #print(inputStream.size())
                #inputOther = connection.makefile('rb')
                outputFile = open(self.filename, 'bw')
                #other2 = self.filename[:-10]
                #other2 = str(other2 + 'hi.txt')
                #print(other2)
                #output2 = open(other2, 'wb')

                #8565402 16772239
                bytesReadSinceSPSHeader = 0
                #buffer2 = inputOther.read(8196)
                buffer = inputStream.read(8196)
                #lenInputStream = len(inputStream.read())
