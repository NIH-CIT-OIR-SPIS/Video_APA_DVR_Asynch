...
class ClientController(threading.Thread):
    ...
    def startRecording(self, _seg_size: int, streamPort: int, serverTime: float) -> None:
        ...
        if not self.recording:
            ...
            if self.previewing and self.camera.resolution == (1280, 720) and self.camera.framerate == 60:
                self.stopPreviewing(serverTime)
            ...
            if self.compression == 1.0 and self.camera.resolution == (1280, 720): 
...
#         self.server.error(self, 'self.camera.zoom: {}'.format(self.camera.zoom))
        ...
        if self.recording:
            if (not self.previewing) and self.camera.resolution == (1280, 720) and self.camera.framerate == 60:
...
#                 self.server.error(self, 'Unable to set frame rate to {}, even after fix attempt. ERROR: {}'.format(fps,err))
    ...
    def setBrightness(self, brightness: int) -> None:
        ...
        :return: Nothing
        """
        self.server.error(self, 'brightness before {}'.format(self.camera.brightness))
        self.camera.brightness = brightness
        self.server.error(self, 'brightness after {}'.format(self.camera.brightness))
        logger.info('Set brightness to {}'.format(brightness))
        
    ##contraster$
    def setContrast(self, contrast: int) -> None:
        self.camera.contrast = contrast
        logger.info('Set contrast to {}'.format(contrast))
...
def clientProtocol(server: ServerConnection,
    ...
    i.assertHandler('set contrast', 'i', client.setContrast)
