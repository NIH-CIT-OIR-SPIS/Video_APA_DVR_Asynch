...
class ClientController(threading.Thread):
    ...
    def __init__(self,
        ...
        #self.zoom = (0, 0, 1920, 1080)
        #self.zoom = (0, 0, 0.3, 0.3)
    ...
    def startRecording(self, _seg_size: int, streamPort: int, serverTime: float) -> None:
        ...
        if not self.recording:
            ...
            self.server.error(self, 'framerate camera {} \n fileStream: {} \n offset {} \n serverTime {} \n splitNotificationTime {}'.format(self.camera.framerate, fileStream, offset, serverTime, splitNotificationTime))
            self.server.error(self, 'resolution Now {}'.format(self.camera.resolution))
            self.server.error(self, 'resolution other Now {}'.format(self.resolution))
            self.server.error(self, 'compression Now {}'.format(self.compression))
#             other=(int(self.resolution[0]/self.compression),
#                     int(self.resolution[1]/self.compression))
#             self.server.error(self, 'resize: {}'.format(other))
    ...
    def startPreviewing(self, previewPort: int, serverTime: float=0) -> None:
        ...
        if not self.previewing:
            ...
            self.server.error(self, 'prv resolution Now {}'.format(self.camera.resolution))
            self.server.error(self, 'prv self.resolution other Now {}'.format(self.resolution))
            self.server.error(self, 'prv compression Now {}'.format(self.compression))
            self.camera.start_recording(
                    self.preview.stdin,
                    format='h264',
                    resize=(int(self.resolution[0]/self.compression),
