...
class ClientController(threading.Thread):
    ...
    def setFPS(self, fps: int) -> None:
        ...
        """
        try:
            self.server.error(self, 'isRecording: {}'.format(self.camera.recording()))
            self.camera.framerate = fractions.Fraction(fps)
            logger.info('Camera fps: {}'.format(fps))
        except Exception as err:
            #try:
            #    self.stopRecording(time.time())
            #    self.camera.framerate = fractions.Fraction(fps)
            #    self.startRecording(time.time())
            #except Exception as err:
            #    self.server.error(self, 'Didnt work was unable to set frame rate of camera to  {}: {}'.format(
            #                fps, err))
            self.server.error(
