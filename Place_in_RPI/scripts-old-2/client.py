...
class ClientController(threading.Thread):
    ...
    def startRecording(self, _seg_size: int, streamPort: int, serverTime: float) -> None:
        ...
        if not self.recording:
            ...
            self.server.error(self, 'Server Info 9-5-20: framerate camera {} \n fileStream: {} \n offset {} \n serverTime {} \n splitNotificationTime {}'.format(self.camera.framerate, fileStream, offset, serverTime, splitNotificationTime))
            self.server.error(self, 'Server Info 9-5-20: resolution Now {}'.format(self.camera.resolution))
            ...
            try:
                ...
                if serverTime:
                    ...
                    self.server.send(self, 'recording stopped',
                                     recordTimestamp)
                    self.server.error(self, 'Server Info 9-5-20: stopped At: {}'.format(recordTimestamp))
